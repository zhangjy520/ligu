...
public class ClassServiceImpl extends BasicController implements ClassService {
    ...
    @Autowired
    A_GradeClassExtensionMapper a_gradeClassExtensionMapper;
...
//        List<ClassSection> defaultList = classSectionMapper.selectByExample(defaultExample);
     ...
     * 班级升级的方法：班级中的
     * 升级规则：（规定时间进行升级）
     * 1.小学一年级  初中一年级 高中一年级 的所有数据重新加入（直接在学籍数据模块中操作）
     * 2.根据学校学段设置 判断是否毕业的班级
     * (1)若该学校只有小学 则六年级的班级在del_flag属性中增加毕业标识2（表示毕业2） 其他班级的年级字段+1
     * (2)若该学校有小学、初中 则六年级的所有班级属性改变-->学段变为初中，年级变为1，其他班级的年级字段+1
     * (3)若该学校有小、初、高，则6年级的规则同(2),九年级的班级中的学段属性变为高中 年级变为1，其他班级的年级字段+1
     *
     * @Author:
     * @params: school_id:学校id  time:班级升级时间
     * @Descrption:
     * @Date:11:43 2018/1/29
     * @Modified By: LL
     */
    @Override
    public void upgradeClass(long upgradeTime, String schoolId) {
        //时间判断(此处规则暂时省略)，逻辑以后补充
        //upgradeTime

        //根据school_id查询所有的学段
        ClassSectionExample classSectionExample = new ClassSectionExample();
        classSectionExample.createCriteria().andDelFlagEqualTo(0).andSchoolIdEqualTo(schoolId);
        List<ClassSection> classSections = classSectionMapper.selectByExample(classSectionExample);
        ...
        List<GradeClass> gradeClassList = gradeClassMapper.selectByExample(gradeClassExample);

        if (null != classSections && classSections.size() > 0) {
            for (ClassSection classSection : classSections) {
                if (null != gradeClassList && gradeClassList.size() > 0) {
                    for (GradeClass gradeClass : gradeClassList) {
                        //小学
                        if (classSection.getSectionYear() == 6) {
                            if (gradeClass.getXd().equals(classSection.getId())) {
                                if (gradeClass.getNj() == 6) {
                                    //是六年级
                                    gradeClass.setDelFlag(2);
                                } else {
                                    //不是六年级
                                    gradeClass.setNj(gradeClass.getNj() + 1);
                                }
                                gradeClassMapper.updateByPrimaryKeySelective(gradeClass);
                            }
                        }
                        //初中或者高中
                        if ((classSection.getSectionYear() == 3&&classSection.getShortName().equals("初中"))
                                || (classSection.getSectionYear() == 3&&classSection.getShortName().equals("高中"))) {
                            if (gradeClass.getXd().equals(classSection.getId())) {
                                if (gradeClass.getNj() == 3) {
                                    gradeClass.setDelFlag(2);
                                } else {
                                    gradeClass.setNj(gradeClass.getNj() + 1);
                                }
