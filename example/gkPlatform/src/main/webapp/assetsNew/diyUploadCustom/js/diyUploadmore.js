...
(function( $ ) {
    ...
    $.fn.extend({
        ...
        */
        diyUpload:function( opt, serverCallBack ) {
            ...
            if(opt.successFinished) {
                var successFinished = opt.successFinished;
                delete opt.successFinished;
            }
    ...
    function createBox( $fileInput, file, webUploader ) {
        ...
        var indexLast = file.name.lastIndexOf(".");
        var fileNameNoSuffix = file.name.substring(0,indexLast);
        var fileNameId = file.name.replace(/[^\w]/g,"A");
        //添加文件名字输入框;
        var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover"> \
					<div class="viewThumb"></div> \
					<div class="diyCancel"></div> \
					<div class="shuom-text"><input type="text" id="'+fileNameId+'" value="'+fileNameNoSuffix+'"></div>\
					<div class="diyBar"> \
