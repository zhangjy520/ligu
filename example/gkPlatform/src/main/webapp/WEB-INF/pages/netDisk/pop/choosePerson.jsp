...
<main class="container clearfix">
    <input name="fileArr" class="fileArr" type="hidden">
    <input name="totalFileUrl" class="totalFileUrl" type="hidden" value="${totalFileUrl}">
    <aside>
        <header>
            <span>姓名</span>
...
<script>
    ...
    function doSubmit() { //回调函数，在编辑和保存动作时，供openDialog调用提交表单。
        ...
        var fileArr = $('input[name="fileArr"]').val();
        window.localStorage.removeItem("${choosesId}");
        $.post("${ctx}/net/disk/share", {
            chooseId: chooseId,
            totalurl: totalurl,
            arr: fileArr
        }, function (data) {
